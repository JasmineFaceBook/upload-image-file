...
public class PersonController {
    ...
    @ResponseBody
    public Object upLoadFile(@RequestParam(value = "commonsMultipartFile", required = false) CommonsMultipartFile commonsMultipartFile,
                             PersonInfoEntity personEntity,
        ...
        try {
            ...
            if (ps.size() > 0) {
                if (null != personEntity.getIsUpdate() && !"".equals(personEntity.getIsUpdate()) && "1".equals(personEntity.getIsUpdate())) {
                    ...
                    mapc.put("areaId", "350000");
                    List<PersonInfoEntity> citylist = personService.getPlace(mapc);
                    modelMap.put("citylist", citylist);
                    //获取这个城市对应的县
                    mapc.put("areaId", null);
                    mapc.put("cityId", personEntity.getCity());
                    List<PersonInfoEntity> countylist = personService.getPlace(mapc);
                    modelMap.put("countylist", countylist);
                    //获取这个县对应的学校
                    mapc.put("areaId", null);
                    mapc.put("cityId", null);
                    mapc.put("countyId", personEntity.getCounty());
                    List<PersonInfoEntity> schoollist = personService.getSchool(mapc);
    ...
    public List<PersonInfoEntity> getCity(ModelMap modelMap) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("areaId", "350000");
        List<PersonInfoEntity> citylist = new ArrayList<>();
        try {
            citylist = personService.getPlace(map);
            modelMap.put("citylist", citylist);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public List<PersonInfoEntity> getCounty(ModelMap modelMap, PersonInfoEntity personEntity) {
        ...
        List<PersonInfoEntity> countylist = personService.getPlace(map);
        modelMap.put("countylist", countylist);
        return countylist;
    }
    ...
    public List<PersonInfoEntity> getSchool(ModelMap modelMap, PersonInfoEntity personEntity) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("countyId", personEntity.getCityId());
